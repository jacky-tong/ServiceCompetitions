...
class DualImageResNet(nn.Module):
    def __init__(self, base_model, num_classes=8):
        ...
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))  # 输出大小为 (1, 1)

        # 获取 base_model 的输出特征维度
        with torch.no_grad():
            # 保证虚拟输入尺寸和实际输入尺寸一致
            dummy_input = torch.randn(1, 3, 448, 448)  
            output = self.base_model(dummy_input)
            output = self.global_avg_pool(output)
            feat_dim = output.view(output.size(0), -1).shape[1]

        self.classifier = nn.Sequential(
            nn.Linear(feat_dim * 2, 512),  # 根据实际特征维度调整输入维度
            nn.ReLU(),
            nn.Dropout(0.3),
    ...
    def forward(self, img1, img2):
        ...
        feat1 = self.global_avg_pool(feat1)
        feat2 = self.global_avg_pool(feat2)
...
def main():
    ...
    base_vgg = vgg16(weights=VGG16_Weights.DEFAULT).features
