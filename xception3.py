...
# 修改 MultiScaleResNet 类以使用 Xception 主干
class MultiScaleResNet(nn.Module):
    def __init__(self, xception_model):
        super(MultiScaleResNet, self).__init__()
        self.xception = xception_model  # 使用 Xception 主干
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        # 可选：添加 SE 模块（假设 Xception 输出通道数为2048）
        self.se = SEBlock(2048)
        
    def forward(self, x):
        # 利用 Xception 的特征提取接口（假设为 forward_features）
        feat = self.xception.forward_features(x)
        feat = self.se(feat)
...
def main():
    ...
    train_indices, val_indices = next(iter(kf.split(range(dataset_size))))
    ...
    import timm  # 新增：使用 timm 加载 Xception 模型
    base_xception = timm.create_model("xception41", pretrained=True)
    base_model = MultiScaleResNet(base_xception)
    ...
    print(f"Final Evaluation: Val Loss: {val_loss:.4f} | Val Acc: {val_accuracy:.4f}")
