...
class FocalLoss(nn.Module):
    def __init__(self, alphas=None, gamma=2, reduction='mean'):
        super(FocalLoss, self).__init__()
        self.alphas = torch.tensor(alphas) if alphas else None
        self.gamma = gamma
    ...
    def forward(self, inputs, targets):
        ...
        if self.alphas is not None:
            alpha_t = self.alphas.to(targets.device)[None, :] * targets + \
                     (1 - self.alphas.to(targets.device)[None, :]) * (1 - targets)
        else:
            alpha_t = 1.0
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=10, save_path="model.pth"):
    ...
    for epoch in range(num_epochs):
        ...
        # 绘制并保存训练和验证的损失和准确率曲线
        os.makedirs("training_plots_finetune", exist_ok=True)
        ...
        plt.savefig(f'training_plots_finetune/loss_curve.png')
        plt.close()
        ...
        plt.savefig(f'training_plots_finetune/accuracy_curve.png')
        plt.close()

    print("\nTraining complete.")

def load_and_finetune(model_path, num_classes=8, train_classifier_only=True, device="cuda"):
    # 1. 初始化基础模型结构
    base_resnet = resnet50(weights=None)  # 不需要预训练权重
    base_model = MultiScaleResNet(base_resnet)
    model = DualImageResNet(base_model=base_model, num_classes=num_classes).to(device)
    
    # 2. 加载已保存的模型参数
    model.load_state_dict(torch.load(model_path))
    model.to(device)  # 假设已定义device
...
def main():
    ...
    model, optimizer = load_and_finetune(model_path=r"D:\Serviceoutsourcingcompetition\newpackage\mine\best_model(7).pth",device=device)
