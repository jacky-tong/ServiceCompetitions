...
def main():
    ...
    # 过采样数据集
    train_dataset_oversampled = DualImageDataset(data_dir=r"D:\Serviceoutsourcingcompetition\newpackage\mine\pretrain2",
                                                 transform=transform["train"])
    train_loader_oversampled = DataLoader(train_dataset_oversampled, batch_size=batch_size, shuffle=True,
    ...
    print("Stage 1: Training on non-oversampled data...")
    train_model(
        model,
        train_loader,
        val_loader,
        ...
        device,
        num_epochs=15,
        save_path="best_model_stage1.pth"
    )

    # 第二阶段：冻结特征提取主干网络，只训练分类头
    ...
    print("Stage 2: Training on oversampled data with frozen feature extractor...")
    train_model(
        model,
        train_loader_oversampled,
        val_loader,
        criterion,
        optimizer,
        scheduler,
